...
public class LeapVRTemporalWarping : MonoBehaviour {
  ...
  private void updateHistory() {
    ...
    _history.Add(new TransformData() {
      leapTime = leapNow,
      localPosition = InputTracking.GetLocalPosition(XRNode.CenterEye),
      localRotation = InputTracking.GetLocalRotation(XRNode.CenterEye)
    });
  ...
  private void updateTemporalWarping() {
    ...
    Vector3 currCenterPos = _trackingAnchor.TransformPoint(InputTracking.GetLocalPosition(XRNode.CenterEye));
    Quaternion currCenterRot = _trackingAnchor.rotation * InputTracking.GetLocalRotation(XRNode.CenterEye);
